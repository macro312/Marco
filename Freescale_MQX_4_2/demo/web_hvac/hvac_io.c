...
bool HVAC_InitializeIO(void)
{
    ...
    /* Init Gpio for Leds as output to drive LEDs (LED10 - LED13) */
       ...
       lwgpio_set_attribute(&button1, LWGPIO_ATTR_PULL_DOWN, LWGPIO_AVAL_ENABLE);
#else
...
#if defined(BSP_BUTTONS_ACTIVE_HIGH)
       lwgpio_set_attribute(&button2, LWGPIO_ATTR_PULL_DOWN, LWGPIO_AVAL_ENABLE);
#else
...
#ifdef BSP_BUTTON3
           input_port = lwgpio_init(&button3, FAN_ON_OFF, LWGPIO_DIR_INPUT, LWGPIO_VALUE_NOCHANGE);
           if(!input_port)
           {
...
#if defined(BSP_BUTTONS_ACTIVE_HIGH)
       lwgpio_set_attribute(&button3, LWGPIO_ATTR_PULL_DOWN, LWGPIO_AVAL_ENABLE);
#else
