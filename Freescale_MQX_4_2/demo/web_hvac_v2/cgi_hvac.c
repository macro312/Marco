...
_mqx_int cgi_hvac_data(HTTPSRV_CGI_REQ_STRUCT* param)
{
    ...
    char* str = NULL;
    char temp_symbol;
    char *fan_mode, *fan_output, *hvac_output, *cool_output, *heat_output, *hvac_mode;
    ...
    Td = HVAC_GetDesiredTemperature();
    Ta = HVAC_GetActualTemperature();
    temp_symbol = HVAC_GetTemperatureSymbol();
    fan_mode = HVAC_GetFanMode() == Fan_Automatic ? "auto" : "on";
    hvac_output = HVAC_GetOutput(HVAC_FAN_OUTPUT) ? "on" : "off";
    cool_output = HVAC_GetOutput(HVAC_COOL_OUTPUT) ? "on" : "off";
    heat_output = HVAC_GetOutput(HVAC_HEAT_OUTPUT) ? "on" : "off";
    hvac_mode = HVAC_HVACModeName(HVAC_GetHVACMode());
