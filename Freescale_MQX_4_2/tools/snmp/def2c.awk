...
function printvarbind(varbind) {
    ...
    if (mibtype[varbind] ~ /INTEGER/) {
        print "   { /* Scope */"
        print "      int32_t   sinteger = 0;"
        print "      uint32_2  uinteger = 0;"
        ...
        print "   { /* Scope */"
        print "      uint32_t   octlen = 0, getoctlen = 0;"
        print "      unsigned char *octstr = NULL;"
        print ""
        print "      switch (node->TYPE) {"
    ...
    if (varbind in mibtableroot) {
        ...
        for (k=0; k<mibtablelen[mibtableroot[varbind]]; k++) {
            if (mibtabletype[mibtableroot[varbind],k] == 0) {
                print "         uint32_t " mibtable[mibtableroot[varbind],k] ";"
            } else {
...
function printmib() {
    ...
    print "/*HEADER****************************************************************"
    print "*"
    print "* Copyright 2008-" strftime("%Y") " Freescale Semiconductor, Inc."
    print "* Copyright 2004-2009 Embedded Access Inc."
    print "* Copyright 1989-2008 ARC International."
    print "*"
    print "* This software is owned or controlled by Freescale Semiconductor."
    print "* Use of this software is governed by the Freescale MQX RTOS License"
    print "* distributed with this Material."
    print "* See the MQX_RTOS_LICENSE file distributed for more details."
    print "*"
    print "* Brief License Summary:"
    print "* This software is provided in source form for you to use free of charge,"
    print "* but it is not open source software. You are allowed to use this software"
    print "* but you cannot redistribute it or derivative works of it in source form."
    print "* The software may be used only in connection with a product containing"
    print "* a Freescale microprocessor, microcontroller, or digital signal processor."
    print "* See license agreement file for full license terms including other restrictions."
    print "***************************************************************************** "
    print "*"
    ...
    for (i in trapent) {
        print "static uint32_t MIB_get_sysUpTime(pointer);"
        print ""
        break
    }

    for (i in mibtablelen) {
        print "boolean MIB_find_" i substr("                   ",length(i)) "(uint32_t, pointer, pointer _PTR_);"
    }

    print ""

    for (i in mibtablelen) {
        print "boolean MIB_instance_" i substr("                   ",length(i)) "(RTCSMIB_WALK_PTR, uint32_t, boolean (_CODE_PTR_)(uint32_t, pointer, pointer _PTR_), boolean _PTR_, pointer _PTR_);"
    }
    ...
    for (i=mibsortindexstart; i<=mibsortindexend; i++) {
        if (i in mibsortindex) {
            ...
            if (mibaccess[s[1]] ~ /WRITE/) {
                print "uint32_t MIB_set_" s[1] substr("                   ",length(s[1])) "(void *, unsigned char *, uint32_t);"
            }
        }
    }
    ...
    for (i in mibtablelen) {
        ...
        print "      uint32_t              op,"
        print "      boolean (_CODE_PTR_  find)(uint32_t, pointer, pointer _PTR_),"
