...
int32_t Shell_play(int32_t argc, char *argv[])
{
    ...
    if ((device_ptr = fopen(argv[1], "w")) == NULL)
    {
    ...
    /* Setup rest of parameters - master clock */
    if (ioctl(device_ptr, IO_IOCTL_I2S_SET_MCLK_FREQ, &mclk_freq) != I2S_OK)
    {
        printf("  Error: Unable to setup \"%s\" master clock.\n", argv[1]);
        fclose(device_ptr);
        fclose(file_ptr);
        return (SHELL_EXIT_ERROR);
    }
    ...
    /* Setup audio codec */
    errcode = SetupCodec(device_ptr, &audio_format);
    if (errcode != 0)
    {
    ...
    ioctl(device_ptr, IO_IOCTL_I2S_GET_TX_STATISTICS, &stats);
    ...
    if (ioctl(device_ptr, IO_IOCTL_I2S_GET_TX_STATISTICS, &stats) != I2S_OK)
    {
...
int32_t Shell_record(int32_t argc, char *argv[])
{
    ...
    if (ram_buffer == NULL)
    {
        printf("No RAM space to do recording!\n\r");
        return SHELL_EXIT_ERROR;
    }
    ...
    ioctl(device_ptr, IO_IOCTL_I2S_GET_RX_STATISTICS, &stats);
    ...
    /* Stop recording */
    ioctl(device_ptr, IO_IOCTL_I2S_STOP_RX, NULL);
    ...
    /* Print results */
    if (ioctl(device_ptr, IO_IOCTL_I2S_GET_RX_STATISTICS, &stats) != I2S_OK)
    {
        printf("  Error: Cannot read I2S statistics.\n");
    }
    else
