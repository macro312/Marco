...
#define INT_BUTTON          BSP_BUTTON1     /*SW4*/
#define INT_BUTTON_FUNC     BSP_BUTTON1_MUX_IRQ   
#define WAKEUP_BUTTON       BSP_BUTTON4     /*SW3*/
#define WAKEUP_BUTTON_FUNC  BSP_BUTTON4_MUX_IRQ
#define WKPU_ID             14              /*SW3 is PTB27, which is WKPU_P14*/
#endif
...
{
    /*
     * SVF522REVB don't have user LEDs
     */
#if !(defined(BSP_SVF522REVB_A5) || defined(BSP_SVF522REVB_M4))
    /*
...
#if defined(BSP_SVF522REVB_A5) || defined(BSP_SVF522REVB_M4)
    /*
     * SVF522REVB don't have user LEDs, so we need other forms of
     * prompt
     */
    uint32_t i;
    printf("Press SW4 to exit speed prompt loop\n");
    for (;;) {
        for (i=0; i!=MAX_SPEED_INDICATOR; i++) {
            my_delay();
            printf(".");
            if (check_button() == BUTTON_PRESSED)
                break;
        }
        if (i != MAX_SPEED_INDICATOR)
            break;
        // Restore Screen
        printf("\r");
        for (i=0; i!=MAX_SPEED_INDICATOR; i++) {
...
#elif defined(BSP_VYBRID_AUTOEVB_A5) || defined(BSP_VYBRID_AUTOEVB_M4)
    printf("Press SW6 to exit LED blink loop...");
#endif
...
{
    /*
     * SVF522REVB don't have user LED
     */
#if !(defined(BSP_SVF522REVB_A5) || defined(BSP_SVF522REVB_M4))
    ...
    )
{
    ...
    defined(BSP_SVF522REVB_A5) || defined(BSP_SVF522REVB_M4)
    /*
    ...
    while (1)
    {
        ...
        "      Indicator doesn't blink\n");

#if defined(BSP_TWR_VF65GS10_A5) ||  defined(BSP_TWR_VF65GS10_M4)
        printf ("\nSetting operation mode to %s, Press SW1 to wakeup... ", get_operation_mode_name (operation_mode));
#elif defined(BSP_VYBRID_AUTOEVB_A5) || defined(BSP_VYBRID_AUTOEVB_M4)
        printf ("\nSetting operation mode to %s, Press SW9 to wakeup... ", get_operation_mode_name (operation_mode));
#elif defined(BSP_SVF522REVB_A5) || defined(BSP_SVF522REVB_M4)
        printf ("\nSetting operation mode to %s, Press SW3 to wakeup... ", get_operation_mode_name (operation_mode));
#endif
