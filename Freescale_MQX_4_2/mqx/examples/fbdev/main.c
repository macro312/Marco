   ...
   )
{
    ...
    uint8_t *buf1;
    uint32_t bytes;
    uint32_t red = 0xFFFF0000, green = 0xFF00FF00;
    ...
    buf_info0.X = 0;
    buf_info0.Y = 0;
    buf_info0.WIDTH /= 2; /* 1/4 screen */
    buf_info0.HEIGHT /= 2;
    buf_info0.FORMAT = FBDEV_COLOR_ARGB8888; /* Set frame buffer format */

    bytes = buf_info0.WIDTH * buf_info0.HEIGHT * 4;

    /* double buffer mode, each 1/4 screen size */
    buf_info0.BUF_COUNT = 2;
    buf_info0.BUFFERS[0] = buf0;
    buf_info0.BUFFERS[1] = buf0 + bytes;

    /* fb1 is at the bottom right */
    buf_info1.WIDTH /= 2; /* 1/4 screen */
    buf_info1.HEIGHT /= 2;
    buf_info1.X = buf_info1.WIDTH;
    buf_info1.Y = buf_info1.HEIGHT;
    buf_info1.FORMAT = FBDEV_COLOR_ARGB8888; /* Set frame buffer format */

    /* double buffer mode, each 1/4 screen size */
    buf_info1.BUF_COUNT = 2;
    buf_info1.BUFFERS[0] = buf1;
    buf_info1.BUFFERS[1] = buf1 + bytes;

    for (i = 0; i < bytes/4; i++)
    {
        memcpy(buf0, &red, 4);    /* buf0 front red */
	buf0 += 4;

        memcpy(buf1, &green, 4);  /* buf1 front green */
	buf1 += 4;
    }

    for (i = 0; i < bytes/4; i++)
    {
        memcpy(buf0, &green, 4);    /* buf0 back green */
	buf0 += 4;

        memcpy(buf1, &red, 4);  /* buf1 back red */
	buf1 += 4;
    }
