...
#if BSP_VYBRID_AUTOEVB_A5 || BSP_SVF522REVB_A5/* AutoEVB */
...
static void setup_layer(MQX_FILE_PTR fd, int layer, DCU4_BPP_TYPE format, int bpp, uint32_t color)
{
    ...
    /* Fill color in the frame buffer */
    buf = (uint8_t *)frame_buffer;
   ...
   )
{
    ...
    setup_layer(fd, 0, DCU_BPP_32, 4, 0x80FF0000); /* red with half transparency */
    ...
    setup_layer(fd, 1, DCU_BPP_16, 2, 0xFC00);          /* orange */
    setup_layer(fd, 2, DCU_BPP_24, 3, 0xFFFF00);        /* yellow */
    setup_layer(fd, 3, DCU_BPP_16_ARGB1555, 2, 0x83E0); /* green */
    setup_layer(fd, 4, DCU_BPP_16_ARGB4444, 2, 0x800F); /* blue with half transparency */
    setup_layer(fd, 5, DCU_BPP_UYVY, 2, 0xCC);          /* purplue */
    ...
    for (i = 0; i < LAYER_NUM; i++)
    {
        ...
        /* hide all RGB/UYVY pixels, only show ARGB color */
        layer_control.LAYER = i;
