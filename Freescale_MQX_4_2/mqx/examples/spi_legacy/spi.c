...
unsigned char   send_buffer[1 + SPI_MEMORY_ADDRESS_BYTES + SPI_MEMORY_PAGE_SIZE] = {0};
unsigned char   recv_buffer[sizeof(TEST_STRING_LONG)] = {0};
...
void set_CS (uint32_t cs_mask, uint32_t logic_level, void *user_data)
{
...
*/
   ...
   spifd = fopen (TEST_CHANNEL_INT, (void *)(SPI_FLAG_HALF_DUPLEX));  // same as NULL
   if (NULL == spifd)
   {
   ...
   /* Write short data to memory */
   memory_write_data_interrupt (spifd, SPI_MEMORY_ADDR3, sizeof(TEST_STRING_SHORT) - 1, (unsigned char *)TEST_STRING_SHORT);
   ...
   /* Page write to memory */
   memory_write_data_interrupt (spifd, SPI_MEMORY_ADDR2, sizeof(TEST_STRING_LONG) - 1, (unsigned char *)TEST_STRING_LONG);
