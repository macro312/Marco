...
int32_t Shell_play_asrc(int32_t argc, char *argv[])
{
    ...
    for (i=0; i<1; i++) {
        ...
        result = ioctl(asrc_pair_fd[i], IO_IOCTL_ASRC_START, NULL);
        ...
        result = ioctl(esai_fd, IO_IOCTL_ESAI_VPORT_START, NULL);
        if (result != 0) {
            printf("Error IO_IOCTL_ESAI_FIFO_START %d!\n", result);
            return -2;
        }
        ESAI_BUFFER_STATUS status;
        printf("ESAI START!\n");
        ...
        {
            uint32_t blocks = 1;
            ioctl(esai_fd, IO_IOCTL_ESAI_VPORT_WAIT_TX_EVENT, &blocks);
            ioctl(esai_fd, IO_IOCTL_ESAI_VPORT_GET_TX_STATUS, &status);
            bytes_read = fread(status.hw_addr, 1, status.block_size, pcm_fd);
            ioctl(esai_fd, IO_IOCTL_ESAI_VPORT_UPDATE_TX_STATUS, &bytes_read);
            if (bytes_read < 0)
                continue;
        }
        ...
        result = ioctl(asrc_pair_fd[i], IO_IOCTL_ASRC_STOP, NULL);
        ...
        result = ioctl(asrc_pair_fd[i], IO_IOCTL_ASRC_UNINSTALL_SRC, NULL);
        ...
        result = ioctl(asrc_pair_fd[i], IO_IOCTL_ASRC_UNINSTALL_DEST, NULL);
...
int32_t Shell_play(int32_t argc, char *argv[])
{
    ...
    ioctl(esai_vport_file, IO_IOCTL_ESAI_VPORT_START, NULL);
    printf("ESAI START!\n");
    ...
    while(!feof(pcm_fd))
    {
        ioctl(esai_vport_file, IO_IOCTL_ESAI_VPORT_WAIT_TX_EVENT, &blocks);
        ioctl(esai_vport_file, IO_IOCTL_ESAI_VPORT_GET_TX_STATUS, &status);
        bytes_read = fread(status.hw_addr, 1, status.block_size, pcm_fd);
        ioctl(esai_vport_file, IO_IOCTL_ESAI_VPORT_UPDATE_TX_STATUS, &bytes_read);
        if (bytes_read < 0)
            continue;
    ...
    ioctl(esai_vport_file, IO_IOCTL_ESAI_VPORT_STOP, NULL);
...
int32_t Shell_record(int32_t argc, char *argv[])
{
    ...
    ret = ioctl(esai_rx_vport, IO_IOCTL_ESAI_VPORT_START, NULL);
    if (MQX_OK != ret) {
        printf("failed to call IO_IOCTL_ESAI_FIFO_START cmd, %d\n", ret);
        return 0;
    }
    ESAI_BUFFER_STATUS rx_status;
    uint32_t blocks = 4;

    while (read_len_total!= 0)
    {
        ioctl(esai_rx_vport, IO_IOCTL_ESAI_VPORT_GET_RX_STATUS, &rx_status);
        ioctl(esai_rx_vport, IO_IOCTL_ESAI_VPORT_WAIT_RX_EVENT, &blocks);
        if (read_len_total > (rx_status.block_size * blocks))
        {
            fwrite(rx_status.hw_addr, rx_status.block_size * blocks, 1, file_ptr);
            read_len = rx_status.block_size * blocks;
            read_len_total -= read_len;
            ioctl(esai_rx_vport, IO_IOCTL_ESAI_VPORT_UPDATE_RX_STATUS, &read_len);
        }
        else
        {
            fwrite(rx_status.hw_addr, read_len_total, 1, file_ptr);
            ioctl(esai_rx_vport, IO_IOCTL_ESAI_VPORT_UPDATE_RX_STATUS, &read_len_total);
            read_len_total = 0;
        }
    }
...
int32_t Shell_record_sai(int32_t argc, char *argv[])
{
    WAVE_FILE_HEADER header;
    ...
    /* Config SAI parameter. */
    SAI_BUFFER_CONFIG buffer_config;
    buffer_config.PERIOD_NUMBER = 2;
    buffer_config.PERIOD_SIZE = 4096;
    if (ioctl(device_ptr, IO_IOCTL_I2S_CONFIG_SAI_RX_BUFFER, &buffer_config) != I2S_OK)
    {
        printf("  Error: Unable to chnage buffer.\n", argv[1]);
        fclose(device_ptr);
        fclose(file_ptr);
        return (SHELL_EXIT_ERROR);
    }
    ...
    audio_format.SAMPLE_RATE = header.FMT_SUBCHUNK.SampleRate;

    /* Set master clock frequency so oversampling = 384 */
    uint32_t mclk_freq = header.FMT_SUBCHUNK.SampleRate * 384;
    /* Set SAI master clock. */
    ...
    /* Start SAI RX */
    if (ioctl(device_ptr, IO_IOCTL_I2S_START_RX, NULL) != I2S_OK)
    {
        printf("  Error: Cannot start SAI Rx.\n");
        fclose(device_ptr);
        fclose(file_ptr);
        return (SHELL_EXIT_ERROR);
    }
    I2S_STATISTICS_STRUCT rx_status;
    while (read_len_total!= 0)
    {
        ioctl(device_ptr, IO_IOCTL_I2S_GET_RX_STATISTICS, &rx_status);
        ioctl(device_ptr, IO_IOCTL_I2S_WAIT_RX_EVENT, NULL);
        if (read_len_total > rx_status.SIZE)
        {
            fwrite(rx_status.OUT_BUFFER, rx_status.SIZE, 1, file_ptr);
            read_len = rx_status.SIZE;
            read_len_total -= read_len;
            ioctl(device_ptr, IO_IOCTL_I2S_UPDATE_RX_STATUS, &read_len);
        }
        else
        {
            fwrite(rx_status.OUT_BUFFER, read_len_total, 1, file_ptr);
            ioctl(device_ptr, IO_IOCTL_I2S_UPDATE_RX_STATUS, &read_len_total);
            read_len_total = 0;
        }
    }
