...
#error This application requires MQX_ENABLE_LOW_POWER to be defined non-zero in user_config.h. Please recompile BSP with this option.
     ...
     #error This application requires MQX_ENABLE_HSRUN defined non-zero in user_config.h. Please recompile BSP with this option.
#endif
    ...
    )
{
    ...
    while (1)
    {
        ...
        /* Switch clock configuration 0 as some platforms do not support
        *  directly from configuration 2 to configuration 3 transition
        */
        if (CM_ERR_OK != _lpm_set_clock_configuration(BSP_CLOCK_CONFIGURATION_0))
        {
          printf("Cannot change clock configuration");
