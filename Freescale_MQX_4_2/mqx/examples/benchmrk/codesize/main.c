...
#error This application and BSP/PSP libraries must be built with specific user_config file
#endif
...
#if MQX_MEM_MONITOR
   {TEST_TASK,   test_task,   1000,  8,  "test", 0,      0,     0    },
#endif
   {0,           0,           0,     0,   0,      0,                 }
};
...
#if MQX_EXIT_ENABLED
#if defined BSP_TWRPXS20 || defined BSP_TWRPXD10
   /* PXD10 & PXS20 doesn't support*/
#else
...
static void io(void)
{
   ...
   IF_TYPICAL(_io_dev_install(NULL,
      (_mqx_int (_CODE_PTR_)(MQX_FILE_PTR, char *, char *))NULL,
      (_mqx_int (_CODE_PTR_)(MQX_FILE_PTR))NULL,
      (_mqx_int (_CODE_PTR_)(MQX_FILE_PTR, char *, _mqx_int))NULL,
      (_mqx_int (_CODE_PTR_)(MQX_FILE_PTR, char *, _mqx_int))NULL,
      (_mqx_int (_CODE_PTR_)(MQX_FILE_PTR, _mqx_uint, void *))NULL,
      NULL));
...
static void ipc(void)
{
   ...
   _ipc_add_io_ipc_handler((_mqx_uint (_CODE_PTR_)(void *))NULL,0);
   _ipc_add_ipc_handler((_mqx_uint (_CODE_PTR_)(void *))NULL,0);
...
static void lwtimer(void)
{
   ...
   IF_SMALL(_lwtimer_add_timer_to_queue(NULL, NULL, 0,
      (void (_CODE_PTR_)(void *))NULL, NULL));
...
#if MQX_CPU != PSP_CPU_MPXD10 // ipsum.S not yet in VLE
...
static void message(void)
{
   ...
   IF_MAX(_msgq_set_notification_function(q, (void (_CODE_PTR_)(void *))NULL, m));
...
#if defined (_ICACHE_INVALIDATE_MBYTES)
...
static void time(void)
{
  ...
  // IF_MAX(_time_normalize_date(NULL));
...
#if MQX_STD_TIME_API
   IF_MAX(mktime(time_date));
   IF_MAX(gmtime_r(0, time_date));
   IF_MAX(localtime_r(0, time_date));
   IF_MAX(timegm(time_date));
#endif
}
...
static void rtc(void)
{
   ...
   IF_MAX(_rtc_init (NULL));
   IF_MAX(_rtc_set_time (0));
   IF_MAX(_rtc_get_time (NULL));
   IF_MAX(_rtc_set_alarm (0, 0));
   IF_MAX(_rtc_get_alarm (NULL));
   IF_MAX(_rtc_callback_reg (NULL, NULL));
   IF_MAX(_rtc_isr (NULL));
#endif
}
...
static void sdcard(void)
{
   ...
   IF_TYPICAL(_io_sdcard_install("sdcard:", (void *)&_bsp_sdcard0_init, NULL));
#endif
}
/*TASK*-----------------------------------------------------
*
* Task Name    : test_task
* Comments     : Test mem allocation
*
*END*-----------------------------------------------------*/
#if MQX_MEM_MONITOR
...
{
    char * test_ptr=NULL;
   test_ptr = (char *)_mem_alloc_system(10000);
   _mem_copy("TEST STRING",test_ptr,12);
   _task_block();
}
#endif
/*TASK*-----------------------------------------------------
...
void Main_task(uint32_t initial_data)
{
    ...
    printf("\n-------------------------------\n");
    printf("\nRAM usage before creating tasks\n");
    _mqx_print_ram_usage();
    test_task_id = _task_create(0, TEST_TASK, 0);
    if (test_task_id == MQX_NULL_TASK_ID) {
       printf ("\n Could not create test_task\n");
    }
    printf("\n-------------------------------\n");
    printf("\nRAM usage after creating tasks\n");
    _mqx_print_ram_usage();
    _task_block();
