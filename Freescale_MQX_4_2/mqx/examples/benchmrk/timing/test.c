...
#if defined(MQX_USE_MEM) && MQX_USE_MEM
	#include <mem_prv.h>
#endif
...
#if defined(_LWMEM) && (defined(MQX_USE_LWMEM) && !MQX_USE_LWMEM)
...
/*#define NO_IO*/
#ifdef NO_IO
_mqx_uint _io_init(void) {return MQX_OK;}
void      _io_serial_default_init(void) {}
_mqx_int  _io_printf( char  *fmt_ptr, ... ) {return 0;}
_mqx_int  _io_sprintf(char  *str_ptr, char  *fmt_ptr, ... ) {return 0;}
_mqx_int  _io_fputc(_mqx_int c, MQX_FILE_PTR p) {return 0;}
_mqx_int  _io_fputs(char * c, MQX_FILE_PTR p) {return 0;}
_mqx_int  _io_putchar(_mqx_int c) { return 0; }
_mqx_int  _io_puts(char *s) { return 0; }
#endif
...
{ /* Body */
   ...
   while (size && td_ptr) {
      printf("TD: 0x%x  ID: 0x%x Next: 0x%x\n", td_ptr, td_ptr->TASK_ID,
         (TD_STRUCT_PTR)((unsigned char *)td_ptr->TD_LIST_INFO.NEXT -
         FIELD_OFFSET(TD_STRUCT,TD_LIST_INFO)
      ));
      size--;
