...
)
{
   ...
   ****************************************************************************/
   if(pipe_descr_ptr->G.SPEED  == USB_SPEED_HIGH)
   {
        ...
        while (node_ptr->member != NULL)
        {
            ...
            {
                     /* TODO: reclaim bandwidth by freeing this SITD */
   ...
   ********************************************************************/
   ...
   /* TODO: the reclaim_band_width cannot work with ITD/SITD, so we are reclaimimg the bw by ourselves */
   if (pipe_descr_ptr->G.SPEED == USB_SPEED_HIGH) 
   {
      ...
      for (i = start; i < usb_host_ptr->FRAME_LIST_SIZE; i+= interval)
      {
            ...
            ********************************************************************/
            for (j = 0; j < 8; j++)
            {
               ...
               if(pipe_descr_ptr->BWIDTH_SLOTS[j])
               {
                  frame_list_bw_ptr[8 * i + j] -= pipe_descr_ptr->BWIDTH;
               }
